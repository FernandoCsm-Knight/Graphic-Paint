...
import FreePolygon from "../_shapes/FreePolygon";
...
const usePolygonDrawing = ({
    ...
    const drawPreview = useCallback((
        ...
        if (pixelated) {
            ...
            for (const pt of pts) {
                const { x: ptx, y: pty } = toPixels(pt, pixelSize);
                ctx.fillRect(ptx, pty, pixelSize, pixelSize);
            }
    ...
    }, [currentColorRef, thicknessRef, pixelated, pixelSize, lineAlgorithm]);
    ...
    }, [contextRef, redrawFromScene, currentColorRef, thicknessRef, pixelated, pixelSize, lineAlgorithm, enterPending, renderViewport]);
