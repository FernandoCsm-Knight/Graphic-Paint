...
/**
 * Rasteriza o contorno de um polígono no modo pixelado.
 * Percorre cada aresta (par de vértices consecutivos) e aplica o algoritmo de
 * Bresenham para desenhar os pixels que compõem cada segmento, invocando
 * `drawPixel` para cada pixel encontrado.
 */
...
/**
 * Desenha o contorno de um polígono no modo padrão (não pixelado) usando a Canvas API.
 * Traça um caminho fechado pelos `points` e aplica `stroke` com o `strokeStyle` e
 * `lineWidth` fornecidos.
 */
...
/**
 * Preenche um polígono no modo pixelado usando scanline.
 * Para cada linha de varredura entre `minY` e `maxY`, calcula as interseções
 * com as arestas do polígono e preenche os pixels entre cada par de interseções,
 * invocando `drawPixel` para cada pixel interior.
 */
